...
public class WebsocketHandler {
    ...
    public WebsocketHandler() {
        authService=new AuthService(SQLDAO.getInstance());
    ...
    public void onMessage(Session session, String message) {
        ...
        if (authToken == null) {
            gameInfo.sendMessage(new ErrorMessage("Error: unauthorized"), new Connection(new AuthData("Unknown"), session, 0));
            return;
        }
    ...
    private AuthData verifyAuthToken(String authToken) {
        try {
            return authService.verifyAuthToken(new AuthData(authToken, ""));
